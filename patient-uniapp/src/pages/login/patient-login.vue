<template>
  <view class="page">
    ...
    <view class="hero">
      <view class="hero-badge">患者端</view>
    ...
    <view class="card">
      ...
      <button class="btn-login" :loading="loading" :disabled="loading" hover-class="btn-login-hover" @click="onLogin">
        登录
      </button>
    </view>
  </view>
...
<script setup>
import { ref } from 'vue'
import { DEV_PREFILL_LOGIN, DEV_LOGIN_PATIENT } from '../../utils/config.js'
...
const phone = ref(DEV_PREFILL_LOGIN ? DEV_LOGIN_PATIENT.phone : '')
const password = ref(DEV_PREFILL_LOGIN ? DEV_LOGIN_PATIENT.password : '')
...
async function onLogin() {
  ...
  loading.value = true
  try {
    await loginWithPassword(p, w, 'patient')
...
.page {
  min-height: 100vh;
  background: linear-gradient(180deg, $color-mint 0%, $bg-page 42%, $bg-page 100%);
}
